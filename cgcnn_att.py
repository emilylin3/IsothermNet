...
# FUNCTIONS
def cumulative_edgeIndex(edge_index, bond_dist):
    ...
    for s in range(S):
        ...
        edge_index_strip = edge_index[s, :, :b]
        edge_index_batched.append(edge_index_strip + (s * N))

        bond_dist_strip = bond_dist[s, :b]
        bond_dist_batched.append(bond_dist_strip)

    edge_index_batched = torch.cat(edge_index_batched, dim=-1).to(device)
    bond_dist_batched = torch.cat(bond_dist_batched, dim=0).to(device)
...
class GraphConvLayer(nn.Module):
    ...
    def forward(self, x_data, edge_index, bond_dist):
        ...
        edge_indx, bond_distance = cumulative_edgeIndex(edge_index, bond_dist)
        ...
        bond_distance = bond_distance / max(bond_distance)
...
class DimProj(nn.Module):
    def __init__(self, dim_in, dim_out):
        ...
        super(DimProj, self).__init__()
        self.dim_in = dim_in
        self.dim_out = dim_out

        self.mlp_project = nn.Linear(self.dim_in, self.dim_out).to(device)
        self.double()
...
# GRAPH ATTENTION + CRYSTAL GRAPH CONVOLUTIONAL NEURAL NETWORK (CGCNN) MODEL => COMBINED GNN MODEL
class GNNModel(nn.Module):
    def __init__(self, structureParams):
        ...
        Parameters:
            dim_in              (1, )    number of input features
            dim_out             (1, )    number of out features
            dim_hidFeat         (1, )    number of hidden features after pooling

            n_convLayer         (1, )    number of graph convolutional layers
            n_attLayer          (1, )    number of graph attention layers
            n_hidLayer_pool     (1, )    number of hidden layers after pooling
        """
        ...
        n_convLayer = len(dim_out)
        n_attLayer = len(dim_att)
        n_hidLayer_pool = len(dim_hidFeat)
        ...
        self.att = nn.ModuleList([GraphAttentionLayer(dim_in=self.dim_in, 
                                                      dim_out=self.dim_att[0], 
                                                      n_heads=n_heads, 
                                                      dropout=dropout)])
        for i in range(1, n_attLayer):
            self.att.append(GraphAttentionLayer(dim_in=self.dim_att[i - 1] * n_heads, 
                                                dim_out=self.dim_att[i], 
                                                n_heads=n_heads,
                                                dropout=dropout))
        ...
        self.conv_hid = self.conv_hid.to(device)

        self.fc_out_isotherm = nn.Linear(self.dim_hidFeat[n_hidLayer_pool - 1], self.dim_isotherm).to(device)
    ...
    def forward(self, x_structure, batchAssign, n_heads):
        ...
        x_node = x_structure[0].to(device)  # (S, N, d)
        x_IM = x_structure[1].to(device)
        x_strucGlobal = x_structure[2].to(device)
        x_bond = x_structure[3].to(device)
        x_textural = x_structure[4].to(device)  # (S, k)
        x_pressure = x_structure[5].to(device)
        ...
        x_strucGlobal = DimProj(dim_in=x_strucGlobal.size(1), dim_out=x_strucGlobal.size(1) * 2)(x_strucGlobal.double())
        x_strucGlobal = nn.ReLU()(x_strucGlobal)

        x_textural = DimProj(dim_in=x_textural.size(1), dim_out=x_textural.size(1) * 2)(x_textural.double())
        ...
        textural_feat_2d = torch.cat(textural_feat_2d, dim=0).to(device)
        pressure_feat_2d = torch.cat(pressure_feat_2d, dim=0).to(device)

        x = x_node.to(device)

        for attLayer in self.att:
            x = x.float()
            x = attLayer(x, edge_index, bond_dist)
            x = x.float()
            x = nn.BatchNorm1d(attLayer.dim_out * n_heads).to(device)(x)
            x = nn.ReLU()(x)
        ...
        for convLayer in self.conv:
            ...
            x = x.float()
            x = nn.BatchNorm1d(convLayer.dim_out).to(device)(x)
            x = nn.ReLU()(x)

        x = global_add_pool(x, batchAssign.to(device))
