...
from config import *
from cgcnn_att import GNNModel
from dataProcessing import *

"""
Training, testing, and validation of GAT-CGCNN model
"""
...
class TrainModel():
    ...
    def __init__(self):
        if train:
            if run_dataProcess:
                _, structureInputs = dataProcessing.run_data_processing(dataDir=dataPath)
                pass
            else:
                print("Loading structural data...", flush=True)
                structureInputs = torch.load(f"{dataPath}X_dataset_electro_xyz_bond_struc.pth")
            ...
            texturalProp_data = pd.read_excel(f"{dataPath}texturalProperties_vol.xlsx")
            ...
            pressureFeat = torch.index_select(pressureFeat, dim=1, index=torch.tensor(pos))

            H_data = torch.load(f"{dataPath}H_dataset.pth")["H"] * -R
            H_data = torch.index_select(H_data, dim=1, index=torch.tensor(pos))
            # if predicting enthalpy, y_data = H_data
            
            loadData = torch.load(f"{dataPath}y_dataset19.pth")
            y_data = loadData["isotherm"]
            y_data = torch.index_select(y_data, dim=1, index=torch.tensor(pos))
            ...
            if len(num) == 1:
                ...
                x_node_train, x_node_val, x_IM_train, x_IM_val, x_strucGlobal_train, x_strucGlobal_val, x_bond_train, x_bond_val, x_textural_train, x_textural_val, x_pressure_train, x_pressure_val, y_train, y_val = train_test_split(
                    nodeFeat, IMFeat, strucGlobalFeat, bondFeat, texturalFeat, pressureFeat, y_data, 
                    test_size=0.2,
                    random_state=42)
                    
                x_node_train, x_node_test, x_IM_train, x_IM_test, x_strucGlobal_train, x_strucGlobal_test, x_bond_train, x_bond_test, x_textural_train, x_textural_test, x_pressure_train, x_pressure_test, y_train, y_test = train_test_split(
                    x_node_train, x_IM_train, x_strucGlobal_train, x_bond_train, x_textural_train, x_pressure_train, y_train, 
                    test_size=0.5, 
                    random_state=42)
    ...
    def tune_hp(self):
        def objective_func(hp_params):
            ...
            structureParams = {
                ...
                "dim_pressureFeat": self.pressureFeat.size(1),

                "dim_in": self.nodeFeat.size(2),
            ...
            model = GNNModel(structureParams).to(device)
    ...
    def calcLoss(self, y_pred_isotherm, y_target_isotherm):
        y_pred_isotherm = y_pred_isotherm.double().to(device)
        y_target_isotherm = y_target_isotherm.double().to(device)
    ...
    def train(self, lr, weight_decay, dropout, n_heads):
        ...
        if load_checkpoint:
            checkpoint = torch.load(os.path.join(f"{savePath}checkpoint{indx_str}.pth"))
        ...
        for epoch in tqdm(range(start_epoch, num_epoch)):
            if iter_stop < train_patience:  # patience
                ...
                torch.save({
                    ...
                    "dropout": dropout,
                    "n_heads": n_heads
                }, os.path.join(f"{savePath}checkpoint{indx_str}.pth"))
                ...
                if (np.mean(val_mae) < best_mae) and (np.mean(val_mse) < best_mse):
                    ...
                    torch.save({
                        ...
                        "dropout": dropout,
                        "n_heads": n_heads
                    }, os.path.join(f"{savePath}best_model{indx_str}.pth"))

                    iter_stop = 0

            else:
                print(f"Model exceeded {train_patience} iterations without improvement. BREAK.")
                break

        return model

    def load_best_model(self, model):
        best_model_path = os.path.join(f"{savePath}best_model{indx_str}.pth")
    ...
    def test(self, model, n_heads):
        ...
        for batch, (x_node_val, x_IM_val, x_strucGlobal_val, x_bond_val, x_textural_val, x_pressure_val, y_data_isotherm) in enumerate(self.test_DataLoader):
            ...
            y_data_isotherm = y_data_isotherm.cpu()
            y_pred_isotherm = y_pred_isotherm.cpu()
        ...
        torch.save({
            ...
            "err_LB": err_LB_arr,
            "err_UB": err_UB_arr
        }, os.path.join(f"{savePath}results{indx_str}.pth"))

        # Plot losses
        plt.figure(figsize=(15, 6))
        data_loss = torch.load(os.path.join(f"{savePath}checkpoint{indx_str}.pth"))
        ...
        for i in range(len(labels)):
            ...
            if i in [0, 4]:
                plt.ylabel("Target Uptake [g/kg]", **font)
        plt.savefig(f"{savePath}parity_textural{indx_str}", dpi=300)
    ...
    def PCA(self):
        ...
        plt.savefig(f"{savePath}pca{indx_str}", dpi=300)
...
if __name__ == "__main__":
    ...
    if load_checkpoint or load_hp:
        # checkpoint = torch.load(f"{savePath}checkpoint{indx_str}.pth"))
